...
***
GC is calculated for a pair, heritability for a single disease. How can I represent that.
```{r}
...
Let's check the pairs that have High Genetic correlation. Do they have high comorbidity as well?
```{r}
don %>% arrange(rg_p) %>% select(D1, D2, CI, rg, rg_se, rg_p)
```


Let's check the pairs that have High Comorbidity. Do they have high GC as well?
```{r}
don %>% arrange(desc(CI)) %>% select(D1, D2, CI, rg, rg_se, rg_p)
```
...
Same thing but with logaritmic X scale
```{r, warning=F}
# Then plot it
p=don %>% 
  #filter(value!=0) %>%
...
range(don$D1_prevalence, na.rm=T)
```
...
Biggest genetic correlation of a given disease?
```{r}
tmp=don %>% 
  filter(D1_code=="F20" ) %>% 
  arrange(desc(rg)) %>%
