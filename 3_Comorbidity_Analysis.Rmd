...
datatable(tmp, rownames = FALSE , width="60%", options = list(pageLength = 40, dom = 't') )
```
...
#Pairs occurence matrix {.tabset .tabset-fade .tabset-pills}
***
...
library(ggridges)
tmp <- ICD %>% select(meaning_L2, short)
length(unique(ICD$meaning_L2))

don <- CI_L2_long %>%
  left_join(tmp, by=c("D1" = "meaning_L2")) %>%
  left_join(tmp, by=c("D2" = "meaning_L2")) %>%
  filter(short.x == short.y)

png("~/Desktop/fig.png", width=800, height=500)
don %>%
  mutate(color=ifelse(short.y=="Mental", "Mental Diseases", "Other")) %>%
  droplevels() %>%
  ggplot( aes(y=reorder(short.y, value), fill=short.y, fill=short.y, x=value+1)) + 
...
#Hierarchical Edge Bundling (between groups) {.tabset .tabset-fade .tabset-pills}
***
...
toremove=c("Chapter XX External causes of morbidity and mortality", "Chapter XXI Factors influencing health status and contact with health services", "Chapter XXII Codes for special purposes")
top = ICD %>% select(meaning_L1) %>% mutate(origine=0) %>% select(origine, meaning_L1) %>% unique %>% filter(! meaning_L1 %in% toremove)
...
A first function that displays the full name of diseases.
```{r}
...
A Second function that displays the letters only name of diseases.
```{r}
plot_bundle2=function(mythres){
  ...
  threshold=mythres
  connexions = CI_L2_long %>% filter(value>threshold) %>% select(D1, D2)
  myvalue = CI_L2_long %>% filter(value>threshold) %>% select(value)
  from = match( connexions$D1, vertices$name)
  to = match( connexions$D2, vertices$name)
  
  # Do the graph
  ggraph(mygraph, layout = 'dendrogram', circular = TRUE) + 
      geom_conn_bundle(data = get_con(from = from, to = to, value=myvalue$value), alpha = 0.7, tension = 1, aes(colour=..index..)) + 
      # Make the size depends how often we see the disease?
      geom_node_point(aes(filter = leaf, x = x*1.05, y=y*1.05, size=val, colour=meaning_L1), alpha=0.6) +
      scale_color_manual(values= rep(brewer.pal(9,"Paired"),9)) +
      scale_size_continuous(range = c(0.3,11)) +
      geom_node_text(aes(x = x*1.11, y=y*1.11, filter = leaf, label=gsub(" .*", "", labels), angle = angle, color=meaning_L1, hjust=hjust), size=1.8, alpha=1) +
      coord_fixed() +
      labs(x="", y="") +
      theme_classic() +
      theme(
...
```{r}
pdf("~/Desktop/img_start.pdf")
# Outside only
  set.seed(123)

  # Do the graph
  ggraph(mygraph, layout = 'dendrogram', circular = TRUE) + 
      geom_edge_diagonal(colour="grey", alpha=0.2) + 
      # Make the size depends how often we see the disease?
      #geom_node_point(aes(filter = leaf, x = x*1.05, y=y*1.05, size=val, colour=meaning_L1), alpha=0.6) +
      scale_color_manual(values= rep(brewer.pal(9,"Paired"),9)) +
      scale_size_continuous(range = c(0.3,11)) +
...
pdf("~/Desktop/img_start_with_nodeValue.pdf")
# Outside only
  set.seed(123)
...
# first connection
pdf("~/Desktop/img_first_connection.pdf")
  set.seed(123)
  
  # Transform the adjacency connection of comorbidity index in 2 vectors: from and to.
  threshold=40
  connexions = CI_L2_long %>% filter(value>threshold) %>% select(D1, D2, value) %>% arrange(desc(value))  %>% filter(value<50)
  myvalue = CI_L2_long %>% filter(value>threshold) %>% select(value) %>% head(1)
  from = match( connexions$D1, vertices$name)
  to = match( connexions$D2, vertices$name)
...
```{r}
pdf("~/Desktop/img_70.pdf")
plot_bundle(70)
dev.off()

pdf("~/Desktop/img_65.pdf")
plot_bundle2(65)
dev.off()

pdf("~/Desktop/img_60.pdf")
plot_bundle2(60)
dev.off()

pdf("~/Desktop/img_55.pdf")
plot_bundle2(55)
dev.off()

pdf("~/Desktop/img_50.pdf")
plot_bundle2(50)
dev.off()

pdf("~/Desktop/img_45.pdf")
plot_bundle2(45)
dev.off()

pdf("~/Desktop/img_40.pdf")
plot_bundle2(40)
dev.off()

pdf("~/Desktop/img_35.pdf")
plot_bundle2(35)
dev.off()

pdf("~/Desktop/img_30.pdf")
plot_bundle2(30)
dev.off()
...
pdf("~/Desktop/img_25.pdf")
plot_bundle2(25)
dev.off()

pdf("~/Desktop/img_20.pdf")
plot_bundle2(20)
dev.off()

pdf("~/Desktop/img_15.pdf")
plot_bundle2(15)
dev.off()

pdf("~/Desktop/img_10.pdf")
plot_bundle2(10)
dev.off()

pdf("~/Desktop/img_5.pdf")
plot_bundle2(5)
dev.off()
```
...
```{r, warning=FALSE}
png("IMG/Edge_Bundle_Como_L2_thres40.png", width=200, height=200, units = "mm", res=300)
plot_bundle(30)
dev.off()
```

![img](IMG/Edge_Bundle_Como_L2_thres40.png)


##Threshold=30

```{r, warning=FALSE}
png("IMG/Edge_Bundle_Como_L2_thres30.png", width=200, height=200, units = "mm", res=300)
plot_bundle(30)
dev.off()
```

![img](IMG/Edge_Bundle_Como_L2_thres30.png)
...
```{r, warning=FALSE}
png("IMG/Edge_Bundle_Como_L2_thres20.png", width=200, height=200, units = "mm", res=300)
plot_bundle(20)
dev.off()
```

![img](IMG/Edge_Bundle_Como_L2_thres20.png)


##Threshold=15

```{r, warning=FALSE}
png("IMG/Edge_Bundle_Como_L2_thres15.png", width=200, height=200, units = "mm", res=300)
plot_bundle(15)
dev.off()
```

![img](IMG/Edge_Bundle_Como_L2_thres15.png)
...
![img](IMG/Edge_Bundle_Como_L2_thres10.png)
...
```{r, warning=FALSE}
png("IMG/Edge_Bundle_Como_L2_thres5.png", width=200, height=200, units = "mm", res=300)
plot_bundle(5)
dev.off()
```

![img](IMG/Edge_Bundle_Como_L2_thres5.png)
...
#Hierarchical Edge Bundling (intra groups) {.tabset .tabset-fade .tabset-pills}
***
...
![img](IMG/Edge_Bundle_Mental.png)
